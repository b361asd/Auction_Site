...
// User
boolean _listActivity = _uri.toUpperCase().contains("listActivity.jsp".toUpperCase());
// User
boolean _listMyBid = _uri.toUpperCase().contains("listMyBid.jsp".toUpperCase());
// User
boolean _listBidForOffer = _uri.toUpperCase().contains("listBidForOffer.jsp".toUpperCase());
// User
boolean _viewAlertDetail = _uri.toUpperCase().contains("viewAlertDetail.jsp".toUpperCase());
// Rep
boolean _listBid = _uri.toUpperCase().contains("listBid.jsp".toUpperCase());
// Rep
boolean _modifyBid = _uri.toUpperCase().contains("modifyBid.jsp".toUpperCase());
int addCol1st = 0;
int addCol2nd = 0;
if (_listActivity) {
    addCol1st = 1;
        ...
        if (_dataTable.rowCount() > 0) {
            for (int i = 0; i < _dataTable.rowCount(); i++) {
                ...
                if (_dataTableBid != null) {
                    ...
                    // Add Addition, minus the above 2
                    out.println(
                            MessageFormat.format(
                                    "<td colspan=''{0}''>",
                                    _dataTable.colCount() + addCol1st - 2));
            ...
            for (int i = 0; i < _dataTable.rowCount(); i++) {
                ...
                if (addCol1st == 1) {
                    ...
                    if (_listActivity || _viewAlertDetail || _listMyBid) {
                        ...
                        if (!__userID.equalsIgnoreCase(_seller)
                                && _status.equalsIgnoreCase("Active")) {
                            out.println(
                                    Helper.getButton(
                                            "form-id-doBid",
                                            "input-id-doBid",
                                ...
                                out.println(
                                        Helper.getButton(
                                                ...
                                                MessageFormat.format(
                                                        "{0},{1},{2}",
                                                        _dataTableBid.getOneCell(j, 0),
                                                        _dataTableBid.getOneCell(j, 1),
                                                        _dataTableBid.getOneCell(j, 2)),
                                                "Modify Bid"));
                            } else if (_modifyBid) {
                                out.println("NEVER");
                            }
