            ...
            for (int i = 0; i < _dataTable.rowCount(); i++) {
                ...
                if (__userType != null && __userType.equalsIgnoreCase("3")) {
                    ...
                    if (!seller.equalsIgnoreCase(__userID) && status.equalsIgnoreCase("Active")) {
                        out.println(
                            Helper.getButton("form-id-doBid", "input-id-doBid", "" + _dataTable.getOneCell(i, 0), "Bid"));
                    }
                    out.println(Helper.getButton("form-id-listBid", "input-id-listBid",
                        _dataTable.getOneCell(i, 0) + "," + _dataTable.getOneCell(i, 2), "List Bid"));
                    if (!_action.equals("listSimilar")) {
                        out.println(Helper.getButton("form-id-listSimilar", "input-id-listSimilar", _dataTable.getOneCell(i, 0) +
                            "," + _dataTable.getOneCell(i, 2) + "," + _dataTable.getOneCell(i, 3), "Similars"));
                    }
                } else {
                    out.println(Helper.getButton("form-id-cancelOffer", "input-id-cancelOffer",
                        "" + _dataTable.getOneCell(i, 0), "Cancel"));
                    out.println(Helper.getButton("form-id-modifyOffer", "input-id-modifyOffer",
                        _dataTable.getOneCell(i, 0) + "," + _dataTable.getOneCell(i, 2) + "," + _dataTable.getOneCell(i, 1),
                        "Modify"));
                }
                out.println("</td>");
                //
